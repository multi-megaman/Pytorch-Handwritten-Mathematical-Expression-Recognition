...
def for_test(x_t):
    ...
    encoder.load_state_dict(torch.load(
        '../model/encoder_lr0.00001_BN_te1_d05_SGD_bs8_mask_conv_bn_b.pkl'))
    attn_decoder1.load_state_dict(torch.load(
        '../model/attn_decoder_lr0.00001_BN_te1_d05_SGD_bs8_mask_conv_bn_b.pkl'))

    encoder.eval()
    attn_decoder1.eval()

    x_t = Variable(x_t.cuda())
    x_mask = torch.ones(x_t.size()[0], x_t.size()[
                        1], x_t.size()[2], x_t.size()[3]).cuda()
    ...
    decoder_attention_t = torch.zeros(
        batch_size_t, 1, dense_input, output_area_t).cuda()
    attention_sum_t = torch.zeros(
        batch_size_t, 1, dense_input, output_area_t).cuda()
